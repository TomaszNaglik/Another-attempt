...
public class Camera extends GameObject
{
	...
	
	{
		...
		{
			Vector3f.add(position, direction, position);
			//Vector3f.sub(direction , position , position);
		}
		if(Input.GetKey(Input.KEY_RIGHT))
		{
			position.GetX().Add+=cameraSpeed;
		}
		if(Input.GetKey(Input.KEY_LEFT))
		{
		...
		*/
		...
		if(locked)
		{
			...
			/*if(rotY)
				yaw += (float) Math.toRadians(deltaPos.GetX() * sensitivity);
			if(rotX)
				pitch += (float) Math.toRadians(-deltaPos.GetY() * sensitivity);
	...
	{
		System.out.print("C: "+transform.GetPos().toString() + "R : " + transform.GetRot().toString());//+" Direction : "+direction.toString()+ " Pitch: "+pitch+" Yaw: "+yaw+" Roll: "+roll);
	}
